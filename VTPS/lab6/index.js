...
async function getUsers(count) {
    ...
    let users = results.map((person) => convertUser(person));

    // const urlExtra = `http://localhost:3000/extraTeachers`;
    // const fetchedExtraData = await fetch(urlExtra);
    // const { extraResults } = await fetchedExtraData.json();
    // console.log(extraResults);
    // const extraUsers = extraResults.map((person) => convertUser(person));
    // users = [...users, ...extraUsers];
    return users;
}


function createStatisticsTable(teachers) {
    ...
    byAgeBtn.onclick = function () {
        ...
        newData.push(filterUsersByCriteria(users).length);
        globalFilter.age = {
            min: 46,
            max: 58
        };
    ...
    byFavoriteBtn.onclick = function () {
        var newData = [];
        globalFilter.favorite = true
        newData.push(filterUsersByCriteria(users).length);
        globalFilter.favorite = false
        newData.push(filterUsersByCriteria(users).length);
        myChart.data.datasets[0].data = newData;
    ...
    bySexBtn.onclick = function () {
        var newData = [];
        globalFilter.gender = "Male"
        newData.push(filterUsersByCriteria(users).length);
        globalFilter.gender = "Female"
        newData.push(filterUsersByCriteria(users).length);
        myChart.data.datasets[0].data = newData;
...
form.onsubmit = async (e) => {
    ...
    users.push(item);
    const url = `http://localhost:3000/extraTeachers`;
    const fetchedData = await fetch(url, {
...
const openTeacherModule = (id) => {
    ...
    const birthdayInThisYear = dayjs(teacher.birthDate).year(currentDate.year());
    const nextBirthday =
        birthdayInThisYear > currentDate ? birthdayInThisYear : birthdayInThisYear.add(1, "year");
    ...
    modalTeacher.querySelector(".nextBd").innerHTML = "Days for Birthday: " + daysToBirthday;
    map.setView([parseFloat(teacher.coordinates.latitude), parseFloat(teacher.coordinates.longitude)], 13)
...
document.querySelectorAll(".paging__button").forEach((button ) => button.addEventListener("click", async (e) => {
